...
var connection = require("./database");
 ...
 router.post("/courts/addnew", function(req,res){
       var   crt_id= req.body.crt_id,
          ...
          pic=req.body.pic,
          est=req.body.est;
      var sql="INSERT INTO courts (crt_id, crt_name, crt_place, crt_type, pic, est) VALUES (?,?,?,?,?,?)";
      connection.query(sql,[crt_id, crt_name, crt_place, crt_type, pic, est], function (error, result) {
        if(error){
 ...
 router.get("/courts/show/:id", function(req,res){
     var id=req.params.id;
     var sql="select * from courts where crt_id = ?";
     connection.query(sql,[id], function (err, result){
         if(err){
             ...
             var obj={print1: result};
             res.render("courts/courtsshow",obj);
         }
     });
});
...
var   crt_id1= req.params.id,
        ...
        crt_type=req.body.crt_type,
         pic=req.body.pic,
          est=req.body.est;
     var sql="UPDATE courts SET crt_id=?, crt_name=?, crt_place=?, crt_type=?, pic=?, est=? WHERE crt_id=?";
     connection.query(sql,[crt_id, crt_name, crt_place, crt_type, pic, est, crt_id1],function(err,result){
         if(err){ 
            ...
            res.redirect("/courts/show/"+ crt_id1);
        }
    });
});
